...
/**
	...
	{
		$config = KTBTrackerConfig::getInstance();
		if (is_null($name)) {
...
**/	
	...
	public static function getUserStatistics($candidate = null, $cycleId = null)
	{
		...
		$stats->requirements = self::getCycleRequirements($cycle);
	...
	public static function getDateInterval($start_date = null, $finish_date = null, $reference_date = null, $date = 'now')
	{
		...
		$interval->week = ($today_intvl->invert) ? intval((($today_intvl->days + 6) / 7) * -1) : intval(($today_intvl->days + 6) / 7);
	...
	public static function getCycleInterval($cycle, $date = 'now', $candidate = null)
	{
		...
		return self::getDateInterval($cycle_start, $cycle_finish, $cycle->cycle_start, $date);
	}
	...
	public static function getThisDate($date = 'now')
	{
		...
		dump("KTBTrackerHelper::getThisDate($date) -> $thisDate");
		return JHtml::_('date', $thisDate);
	}
	...
	public static function getPrevDate($date = 'now')
	{
		...
		dump("KTBTrackerHelper::getPrevDate($date) -> $prevDate");
		return JHtml::_('date', $prevDate);
	}
	...
	public static function getNextDate($date = 'now')
	{
		...
		dump("KTBTrackerHelper::getNextDate($date) -> $nextDate");
		return JHtml::_('date', $nextDate);
	}
	...
	public static function getThisSunday($date = 'now')
	{
		...
		if (date('w', $thisDate) == 0) {
			$thisWeek = strtotime('+0 days', $thisDate);
		} else {
			$thisWeek = strtotime('last sunday', $thisDate);
		}
	...
	public static function getThisWeekDate($date = 'now', $weekStart = 6)
	{
		...
		dump("KTBTrackerHelper::getThisWeekDate($date, $weekStart) -> $thisDate");
		return JHtml::_('date', $thisDate, 'Y-m-d');
	}
	
	public static function getPrevWeekDate($date = 'now', $weekStart = 6)
	{
		$prevDate = date('Y-m-d', strtotime('-1 week', self::getThisWeek($date, $weekStart)));
	
		dump("KTBTrackerHelper::getPrevWeekDate($date, $weekStart) -> $prevDate");
		return JHtml::_('date', $prevDate, 'Y-m-d');
	}
	
	public static function getNextWeekDate($date = 'now', $weekStart = 6)
	{
		$nextDate = date('Y-m-d', strtotime('+1 week', self::getThisWeek($date, $weekStart)));
	
		dump("KTBTrackerHelper::getNextWeekDate($date, $weekStart) -> $nextDate");
		return JHtml::_('date', $nextDate, 'Y-m-d');
	}
	...
	public static function getWeekDates($date = 'now', $weekStart = 6)
	{
		...
		$weekDates[0] = JHtml::_('date', date('Y-m-d', $start), 'Y-m-d');
		$weekDates[1] = JHtml::_('date', date('Y-m-d', strtotime('+1 days', $start)), 'Y-m-d');
		$weekDates[2] = JHtml::_('date', date('Y-m-d', strtotime('+2 days', $start)), 'Y-m-d');
		$weekDates[3] = JHtml::_('date', date('Y-m-d', strtotime('+3 days', $start)), 'Y-m-d');
		$weekDates[4] = JHtml::_('date', date('Y-m-d', strtotime('+4 days', $start)), 'Y-m-d');
		$weekDates[5] = JHtml::_('date', date('Y-m-d', strtotime('+5 days', $start)), 'Y-m-d');
		$weekDates[6] = JHtml::_('date', date('Y-m-d', strtotime('+6 days', $start)), 'Y-m-d');
		
		dump($weekDates, "KTBTrackerHelper::getWeekDates($date, $weekStart)");
		return $weekDates;
	}
	...
	public static function getCycleRequirements($cycle = null)
	{
		return array_merge(self::getPhysicalRequirements($cycle), self::getClassRequirements($cycle), self::getOtherRequirements($cycle));
	}
	
	public static function getCycleRequirementsX($cycle, $key_prefix = 'rqmnt')
	{
		// Create a new query object.
		$db = JFactory::getDbo();
		$query = 'SELECT \''.$key_prefix.'_blog\' AS name, id, ' .
				'requirement, title, description, access, carryover_factor, calcbase_days, repeatable, tract ';
		$query .= 'FROM #__ktbtracker_requirements ';
		$query .= 'WHERE id = ' . (int) $cycle->reqid_blog;
		for ($i = 0; $i < 32; $i++) {
			$key = 'reqid' . $i;
			if (!empty($cycle->$key)) {
				$subquery = 'SELECT \''.$key_prefix.$i.'\' AS name, id, ' .
						'requirement, title, description, access, carryover_factor, calcbase_days, repeatable, tract ';
				$subquery .= 'FROM #__ktbtracker_requirements ';
				$subquery .= 'WHERE id = '.(int) $cycle->$key;
				$query .= ' UNION ' . $subquery;
			} else {
				$subquery = 'SELECT \''.$key_prefix.$i.'\' AS name, 0 AS id, ' .
						'0 AS requirement, NULL AS title, NULL AS description, 0 AS access, 0 AS carryover_factor, ' .
						'0 AS calcbase_days, 0 AS repeatable, 0 AS tract';
				$query .= ' UNION ' . $subquery;
			}
		}
		$db->setQuery($query);
		$requirements = $db->loadObjectList('name');
	
		return $requirements;
	}
	
	public static function getCurrentCycleX($trackingUser = null)
	{
		$cycleid = self::getCurrentCycleId($trackingUser);
	
		if (empty($cycleid)) {
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id');
			$query->from('#__ktbtracker_cycles');
			if (!empty($trackingUser)) {
				$query->where('id IN (SELECT cycleid ' .
						'FROM #__ktbtracker_candidates ' .
						'WHERE userid = '.$trackingUser.')');
			}
			$query->order('state DESC, cycle_start DESC');
			$db->setQuery($query, 0, 1);
			$cycleid = $db->loadResult();
		}
	
		// Get the cycle
		$cycle = self::getCycle($cycleid);
	
		// If we failed to find a cycle for the candidate, just get the most recent
		if (empty($cycle)) {
			$cycle = self::getCycle(self::getCurrentCycleId());
		}
	...
	public static function getUserTotals($candidate)
	{
		...
		$query->select(
				'SUM(a.reqmnt0) AS reqmnt0, ' .
				'SUM(a.reqmnt1) AS reqmnt1, ' .
				'SUM(a.reqmnt2) AS reqmnt2, ' .
				'SUM(a.reqmnt3) AS reqmnt3, ' .
				'SUM(a.reqmnt4) AS reqmnt4, ' .
				'SUM(a.reqmnt5) AS reqmnt5, ' .
				'SUM(a.reqmnt6) AS reqmnt6, ' .
				'SUM(a.reqmnt7) AS reqmnt7, ' .
				'SUM(a.reqmnt8) AS reqmnt8, ' .
				'SUM(a.reqmnt9) AS reqmnt9, ' .
				'SUM(a.reqmnt10) AS reqmnt10, ' .
				'SUM(a.reqmnt11) AS reqmnt11, ' .
				'SUM(a.reqmnt12) AS reqmnt12, ' .
				'SUM(a.reqmnt13) AS reqmnt13, ' .
				'SUM(a.reqmnt14) AS reqmnt14, ' .
				'SUM(a.reqmnt15) AS reqmnt15, ' .
				'SUM(a.reqmnt16) AS reqmnt16, ' .
				'SUM(a.reqmnt17) AS reqmnt17, ' .
				'SUM(a.reqmnt18) AS reqmnt18, ' .
				'SUM(a.reqmnt19) AS reqmnt19, ' .
				'SUM(a.reqmnt20) AS reqmnt20, ' .
				'SUM(a.reqmnt21) AS reqmnt21, ' .
				'SUM(a.reqmnt22) AS reqmnt22, ' .
				'SUM(a.reqmnt23) AS reqmnt23, ' .
				'SUM(a.reqmnt24) AS reqmnt24, ' .
				'SUM(a.reqmnt25) AS reqmnt25, ' .
				'SUM(a.reqmnt26) AS reqmnt26, ' .
				'SUM(a.reqmnt27) AS reqmnt27, ' .
				'SUM(a.reqmnt28) AS reqmnt28, ' .
				'SUM(a.reqmnt29) AS reqmnt29, ' .
				'SUM(a.reqmnt30) AS reqmnt30, ' .
				'SUM(a.reqmnt31) AS reqmnt31'
				);
		...
		$query->where('a.tracking_date BETWEEN ' .
				'DATE('.$db->quote(substr($intvl->cycle_start, 0, 10)).')' .
				' AND ' .
				'DATE('.$db->quote(substr($intvl->cycle_finish, 0, 10)).')');
		$db->setQuery($query);
		...
		$query->where('created BETWEEN '.
				$db->quote($intvl->cycle_start).' AND '.$db->quote($intvl->cycle_finish).' + INTERVAL 1 DAY');
		$query->group('1');
		...
		// Get the number of days where at least one journal entry was made
		$totals->reqmnt_blog = $db->getNumRows();
