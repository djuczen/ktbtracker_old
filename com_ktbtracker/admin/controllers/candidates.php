...
/**
 * KTBTracker component task controller for candidate lists (Administration).
 * 
 * @since	1.0.0
*/
class KTBTrackerControllerCandidates extends JControllerAdmin
{
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_KTBTRACKER_CANDIDATES';
	...
	public function setStatus()
	{
		...
		// Access checks.
		foreach ($ids as $i => $id) {
			if (!$user->authorise('core.edit.state', 'com_ktbtracker.candidate.' . (int) $id)) {
		...
		$this->setRedirect(JRoute::_('index.php?option=com_ktbtracker&view=candidates', false), $message);
	}
	 ...
	 * @since   1.0.0
	 */
	public function getModel($name = 'Candidate', $prefix = 'KTBTrackerModel', $config =  array('ignore_request' => true)) 
	{
